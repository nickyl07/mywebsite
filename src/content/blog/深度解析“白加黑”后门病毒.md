...
![202406141857419](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221222651145.png)

- **真实面目**：这是一个快捷方式（.lnk）文件。

![image-20260121143249488](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221222653363.png)
...
![image-20260121145525290](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221222706295.png)

### 2. “白加黑”运作流程

- **白文件**：`Au.exe`。这实际上是腾讯公司的一款合法程序，拥有有效的数字签名 。在杀毒软件看来，这是“良民”。

![image-20240616230713247](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221222708565.png)
...
- **漏洞利用**：`Au.exe` 在运行时会尝试加载名为 `TenioDL_core.dll` 的库文件。由于它没有校验该DLL的路径或签名，病毒便将恶意DLL放在 `Au.exe` 同级目录下。根据Windows的加载顺序，`Au.exe` 优先加载了恶意的DLL，从而让恶意代码在合法进程的内存空间中运行 。
  ...
  ![202406191818591](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224118362.png)
...
![202406191817961](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224211435.png)

- **执行**：病毒在内存中解密该文件，这实际上是一个PE（可执行）结构的代码，解密后直接在内存中执行，避免了在硬盘上留下明显的恶意EXE文件 。

  通过xdbg动态调试，然后dump解密后的内存。文件大小通过.\config.dat 的大小确认64CF0。

  ![202406261624026](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224212931.png)

  ![202406261627843](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224215367.png)

  1， 选择 插件/Scylla 打开插件窗口

  2， 选择 File/ Dump memory

  ![202406261809405](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224217081.png)
  ...
  ![202406261704821](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224218869.png)
...
- **窃取令牌**：它会查找桌面进程 `explorer.exe`，并窃取其用户令牌（Token）。 
  ...
  ![image-20260121215201042](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224221512.png)

- **模拟身份**：利用 `DuplicateTokenEx` 和 `CreateProcessAsUserA`，病毒从原本可能的系统权限（System）“降维”伪装成当前登录的用户 。 

![image-20260121215502045](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224222939.png)
...
## 第四阶段：狡猾的持久化
  ...
  ![202406191817935](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224225764.png)
  ...
  ![image-20260121220201690](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224228384.png)

### 3. 反安全软件

**针对金山毒霸**：采取“暴力”策略。 代码中显式调用 `KillProcessByName`，试图直接结束金山毒霸（`KSafeTray.exe`）的进程 。 

![image-20260121210851969](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224230017.png)

**针对 360安全卫士**：采取“隐忍”策略。 如果检测到 360（`360tray.exe`）在运行，它会**直接放弃后续的注册表启动项写入**。 病毒选择“认怂”以避免触发 360 的主动防御弹窗，宁可放弃开机自启，也要保全当前在内存中的运行 。

![image-20260121214247583](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224231579.png)
...
![image-20260121221236527](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224233597.png)
...
![image-20260121221220327](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224235424.png)

### 3. 多重套娃

病毒甚至还释放了另一组“白加黑”组合：`load.exe`（另一个腾讯签名的白文件）配合 `dllhost.exe`（恶意代码），进一步下载或执行攻击者指定的命令 。

![image-20260121221951964](https://image-hosting-210.oss-cn-beijing.aliyuncs.com/blog/20260221224237710.png)
